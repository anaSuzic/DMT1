    ...
    | ⟨ wy, pfwy ⟩  =>
        refine          -- like exact but proof can have holes
          ...
          Exists.intro
            ...
            sorry  -- a proof that it is equal to 10 (you!) -- I am not sure if I need to replace this, but I assume we would replace it with 'exact pfwy'
...
example : ∃ (s : Student), Takes s CS1 := by
  exact ⟨Mary, mt1⟩
...
example : ∀ (s : Student), Takes s CS2 → Takes s CS1 := by
  ...
  cases s with
  | Mary => exact mt1
  | Tom   => cases h   -- impossible, no constructor for Takes Tom CS2
  | Carla => cases h   -- impossible, no constructor for Takes Carla CS2

example : ¬(∀ (s : Student), Takes s CS1 → Takes s CS2) := by
  intro H
  -- show contradiction using Tom: Tom takes CS1 but not CS2
  have notTom : ¬ Takes Tom CS2 := by
    intro ht
    cases ht
  exact notTom (H Tom tt1)
