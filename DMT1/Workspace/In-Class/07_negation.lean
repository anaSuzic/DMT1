...
-- Write a function definition that takes two arguments
-- First argument is the type of P -> False
-- Second argument is the proof of P
...
-- Same thing with the NOT (¬) operator
def bar {P : Prop} {α : Type}: ¬P → P → α
| np, p => nomatch (np p)


-- having a proof of NOT something is always a function
